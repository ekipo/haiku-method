...
**Baseline-update policy across all four outcomes:** The reaction mechanism only produces correct eventual-consistency behavior if every classification outcome — including the non-terminal ones — has a defined effect on the baseline. Without this, drift is re-detected on every subsequent tick until a downstream action resolves, producing a steady-state loop that violates the success criterion in DISCOVERY.md § "Success criteria: Outcome-based" ("humans stop circumventing the framework; the framework is now the path of least resistance"). DISCOVERY.md § "Risks: Classification gets stuck in a loop" surfaces this risk and names two candidate strategies; that risk must be answered at inception, not deferred silently.

The baseline-update contract by outcome is:

- **ignore** — Baseline is updated immediately to the observed file state. The change is acknowledged with no further action; the next tick observes no drift on this file.
- **inline-fix** — Baseline is updated immediately to the observed file state. The agent treats the human's edit as the new input for the current bolt; the next tick observes no drift on this file.
- **surface-as-feedback** — A pending-assessment marker is recorded for this file alongside the feedback item. The drift-detection gate skips files with an open pending-assessment marker on subsequent ticks, so the same finding is not re-surfaced. When the feedback item closes (resolved or rejected), the marker is cleared and the baseline is updated to whatever state the file holds at that moment. The baseline is NOT updated at classification time; it is updated at feedback closure.
- **trigger-revisit** — A pending-assessment marker is recorded for this file alongside the revisit dispatch. The gate skips files with an open marker on subsequent ticks. When the revisit completes (the targeted stage re-passes its gate), the marker is cleared and the baseline is updated to the state the file holds at revisit completion. The baseline is NOT updated at classification time; it is updated at revisit completion.

This stance preserves the invariant that "the baseline always reflects either an agent-written state or an agent-acknowledged human state, never an unacknowledged human state." For non-terminal outcomes, the pending-assessment marker is the explicit suppression mechanism — a parallel concept to the baseline itself, not an extension of it. The exact storage location of the marker (inline in the baseline record, alongside the feedback file, in a sidecar log) is a design-stage decision (see "Pending-assessment marker storage" in Open for Design below).

This decision rejects the alternative stance ("update the baseline immediately on every classification, regardless of outcome") because it conflates "agent has seen this change" with "the change is resolved." Under that alternative, a `surface-as-feedback` finding records the file as agent-acknowledged at classification time, and a subsequent unrelated drift on the same file (a second human edit while the feedback is still open) would be diffed against the post-edit baseline rather than the original — which loses the original drift signal that motivated the feedback. The pending-assessment-marker stance preserves the original drift signal for the duration of the unresolved action.
...
**Pending-assessment marker storage** — The baseline-update contract in Decision 3 names a "pending-assessment marker" that suppresses re-detection of drift on a file while a `surface-as-feedback` or `trigger-revisit` outcome is still unresolved. Where does this marker live, how is it keyed (file path + assessment-record ID, or file path alone), and how is its lifecycle tied to the underlying feedback or revisit completion event? Candidate storage sites parallel the baseline storage choices (inline in the baseline record, a sidecar field alongside the feedback file, an entry in the assessment-record log). The choice affects how cleanly the gate can perform its "skip if pending" check on every tick. Design must commit to a marker storage location and lifecycle contract before Plugin Surface 3 (drift-detection gate) and Plugin Surface 4 (manual_change_assessment action) can be implemented.
