...
- **Gap:** DATA-CONTRACTS.md §4.4 + §6 + `pending_marker_schema.feature` were the outliers — they specified `addressed` as a clearance trigger when the ratified product spec (unit-01 AC-G5/AC-SF3 and DATA-CONTRACTS.md §3 narrative at line 414) had already chosen the conservative path: only terminal states (`closed`, `rejected`) clear the marker.
- **Comparable working sibling:** the unit-01 acceptance criteria + the §3 narrative in DATA-CONTRACTS.md ("clearance fires when the linked feedback transitions to a terminal state (closed or rejected)") already state the conservative contract explicitly with rationale.
...
- `stages/product/outputs/DATA-CONTRACTS.md` §4.4 — rewrote Purpose + R5 trigger contract; trigger enum now `"feedback-closed" | "feedback-rejected" | "revisit-complete"` (dropped `"feedback-addressed"`); added explicit cross-references to `pending_marker_schema.feature`, `manual-change-assessment.feature`, and unit-01.
- `stages/product/outputs/DATA-CONTRACTS.md` §6 (`pending_marker_cleared` event) — same enum tightening + worked-example fix (`"trigger": "feedback-closed"`).
...
- `features/manual-change-assessment.feature` — converted the terminal-state scenario to a Scenario Outline parameterized over `closed | rejected`; added an explicit "addressed does NOT clear" scenario with rationale.

All four artifacts now agree on the same trigger contract.

## Status

Closed by bolt 2 — single normative decision propagated consistently across DATA-CONTRACTS.md §4.4 + §6, `pending_marker_schema.feature`, `manual-change-assessment.feature`, and unit-01 acceptance criteria.
