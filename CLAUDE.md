...
| Upstream Finding | Quality Enforcement | `upstream_stage:` on feedback frontmatter — signals root-cause stage is different from review stage; workflow surfaces via `upstream_finding_surfaced` rather than fixing | state-tools.ts, orchestrator.ts |
