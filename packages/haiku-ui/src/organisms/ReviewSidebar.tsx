...
export function ReviewSidebar({
		...
		try {
			...
			const feedback = allComments
				.map((c) =>
					c.type === "general"
						? c.comment
						: `[${c.type}] "${truncate(c.text, 40)}": ${c.comment}`,
				)
				.filter((s) => s.length > 0)
				.join("\n\n")
	...
	async function doExternalReview() {
				...
				{comments.map((c) => (
					...
					>
						...
						{editingId === c.id ? (
							...
							<div onClick={(e) => e.stopPropagation()}>
								...
								<div className="flex justify-end gap-1 mt-1">
									<button
										...
										className="px-2 py-0.5 text-xs text-stone-500 hover:bg-stone-100 dark:hover:bg-stone-700 rounded"
									>
										Cancel
									</button>
									<button
							...
							<>
								{c.comment && (
									<p className="text-xs text-stone-700 dark:text-stone-300 mt-0.5 line-clamp-3">
										{c.comment}
									</p>
								)}
					...
					<div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm">
						<div className="bg-white dark:bg-stone-900 rounded-xl border border-stone-200 dark:border-stone-700 shadow-2xl p-6 max-w-sm mx-4">
							...
							<p className="text-sm text-stone-600 dark:text-stone-400 mb-4">
								This will advance to the next stage and submit the work for
								external review (PR, MR, etc.). Run{" "}
								<code className="px-1 py-0.5 bg-stone-100 dark:bg-stone-800 rounded text-xs">
									/haiku:pickup
								</code>{" "}
								after external approval to continue.
							</p>
				...
				{showApproveConfirm && (
					<div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm">
						<div className="bg-white dark:bg-stone-900 rounded-xl border border-stone-200 dark:border-stone-700 shadow-2xl p-6 max-w-sm mx-4">
							...
							<p className="text-sm text-stone-600 dark:text-stone-400 mb-4">
								You have pending feedback. Approving will discard all comments
								and annotations.
							</p>
							<div className="flex gap-3">
								<button
