...
- A stage's retry budget is TIGHT: agent-invoked rejection cycles are capped at <%= maxStageIterations %> iterations (`MAX_STAGE_ITERATIONS=<%= maxStageIterations %>`). Beyond that, the framework escalates to the human — repeated rejections indicate a spec problem the pre-execute review should have caught, and the correct response is to fix the plan, not keep building against a broken plan.
