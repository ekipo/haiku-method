// orchestrator/prompts/decompose.ts — Per-stage unit-spec writing.
// Renamed from `elaborate.ts` (2026-05-08) when the per-stage cursor
// action was split into two phases:
//   - `elaborate` — the human-conversation gate (new, separate file).
...
// any autonomous decomposition work fires. The cursor blocks at
// `elaborate` until the conversation artifact is captured AND
// verified for substance; only then does it fall through to
// `decompose`. Autopilot mode bypasses the conversation gate and
// walks straight here.
//
// Original header:
