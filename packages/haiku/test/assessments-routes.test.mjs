...
mkdirSync(join(intentDirPath, "stages", stage1, "drift-assessments"), { recursive: true })
mkdirSync(join(intentDirPath, "stages", stage2, "drift-assessments"), { recursive: true })
...
writeFileSync(
  ...
  JSON.stringify({ stage: stage1, status: "active", phase: "execute", visits: 0 }),
)
writeFileSync(
  join(intentDirPath, "stages", stage2, "state.json"),
  JSON.stringify({ stage: stage2, status: "active", phase: "execute", visits: 0 }),
)
...
const da1 = {
  ...
  findings: [{ path: "stages/design/artifacts/hero.html", stage: stage1, change_kind: "modified" }],
  classifications: [{ outcome: "ignore", path: "stages/design/artifacts/hero.html" }],
  agent_rationale: "Minor tweak.",
}
const da2 = {
  id: "DA-02",
  created_at: "2026-04-02T10:00:00Z",
  findings: [{ path: "stages/development/artifacts/app.ts", stage: stage2, change_kind: "modified" }],
  classifications: [{ outcome: "surface-as-feedback", path: "stages/development/artifacts/app.ts" }],
  agent_rationale: "Substantial change.",
}
const da3 = {
  id: "DA-03",
  created_at: "2026-04-03T10:00:00Z",
  findings: [{ path: "stages/design/artifacts/layout.html", stage: stage1, change_kind: "new-file-detected" }],
  classifications: [{ outcome: "inline-fix", path: "stages/design/artifacts/layout.html" }],
  agent_rationale: "New layout file added.",
}

writeFileSync(join(intentDirPath, "stages", stage1, "drift-assessments", "DA-01.json"), JSON.stringify(da1))
writeFileSync(join(intentDirPath, "stages", stage2, "drift-assessments", "DA-02.json"), JSON.stringify(da2))
writeFileSync(join(intentDirPath, "stages", stage1, "drift-assessments", "DA-03.json"), JSON.stringify(da3))
...
async function run() {
  ...
  await test("Filters by stage — only design stage assessments", async () => {
    const res = await fetch(`${baseUrl}/api/intents/${intentSlug}/assessments?stage=${stage1}`)
  ...
  await test("Filters by outcome — only surface-as-feedback", async () => {
    const res = await fetch(`${baseUrl}/api/intents/${intentSlug}/assessments?outcome=surface-as-feedback`)
    assert.strictEqual(res.status, 200)
    const data = await res.json()
    assert.ok(data.ok)
  ...
  await test("limit parameter caps results", async () => {
    const res = await fetch(`${baseUrl}/api/intents/${intentSlug}/assessments?limit=2`)
    assert.strictEqual(res.status, 200)
    const data = await res.json()
    assert.strictEqual(data.assessments.length, 2)
    assert.strictEqual(data.total, 3)
  ...
  await test("Returns 400 for invalid limit", async () => {
    const res = await fetch(`${baseUrl}/api/intents/${intentSlug}/assessments?limit=abc`)
  ...
  await test("Single assessment GET returns full record", async () => {
    const res = await fetch(`${baseUrl}/api/intents/${intentSlug}/assessments/DA-01`)
    assert.strictEqual(res.status, 200)
    const data = await res.json()
    assert.ok(data.ok)
  ...
  await test("Returns 404 for missing DA-*.json file", async () => {
    const res = await fetch(`${baseUrl}/api/intents/${intentSlug}/assessments/DA-99`)
    assert.strictEqual(res.status, 404)
    const data = await res.json()
    assert.ok(data.error === "assessment_not_found" || data.code === "assessment_not_found")
  })

  await test("Returns 404 for invalid assessment ID format", async () => {
    const res = await fetch(`${baseUrl}/api/intents/${intentSlug}/assessments/not-a-valid-id`)
    assert.strictEqual(res.status, 404)
    const data = await res.json()
    assert.ok(data.error === "assessment_not_found" || data.code === "assessment_not_found")
  })
  ...
  await test("Returns 404 for missing intent on single GET", async () => {
    const res = await fetch(`${baseUrl}/api/intents/no-such-intent/assessments/DA-01`)
    assert.strictEqual(res.status, 404)
    const data = await res.json()
    assert.ok(data.error === "intent_not_found" || data.code === "intent_not_found")
  })
