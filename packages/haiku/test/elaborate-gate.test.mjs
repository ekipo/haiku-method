...
test("elaborate gate: fires on fresh stage when mode is continuous", async () => {
	if (!HAS_GIT) return
	await withTmpRepo("elab-gate-fresh", async ({ repoRoot, intentDir, slug }) => {
...
test("elaborate gate: bypassed in autopilot mode", async () => {
	if (!HAS_GIT) return
	await withTmpRepo("elab-gate-autopilot", async ({ repoRoot, intentDir, slug }) => {
		makeStudio({ repoRoot, studio: "test" })
		makeIntent({
			intentDir,
			slug,
...
test("elaborate_review: fires when artifact exists but unverified", async () => {
	if (!HAS_GIT) return
	await withTmpRepo("elab-review-unverified", async ({ repoRoot, intentDir, slug }) => {
		makeStudio({ repoRoot, studio: "test" })
		makeIntent({ intentDir, slug, studio: "test", mode: "continuous" })
...
test("elaborate gate: clears when artifact is verified", async () => {
	if (!HAS_GIT) return
	await withTmpRepo("elab-gate-verified", async ({ repoRoot, intentDir, slug }) => {
		makeStudio({ repoRoot, studio: "test" })
		makeIntent({ intentDir, slug, studio: "test", mode: "continuous" })
...
test("elaborate gate: grandfathered when units already exist", async () => {
	if (!HAS_GIT) return
	await withTmpRepo("elab-gate-grandfather", async ({ repoRoot, intentDir, slug }) => {
		makeStudio({ repoRoot, studio: "test" })
		makeIntent({ intentDir, slug, studio: "test", mode: "continuous" })
...
test("pre-intent verifier: fires when intent.md lacks verified_at", async () => {
	if (!HAS_GIT) return
	await withTmpRepo("preintent-verify", async ({ repoRoot, intentDir, slug }) => {
		makeStudio({ repoRoot, studio: "test" })
...
test("pre-intent verifier: bypassed in autopilot mode", async () => {
	if (!HAS_GIT) return
	await withTmpRepo("preintent-autopilot", async ({ repoRoot, intentDir, slug }) => {
		makeStudio({ repoRoot, studio: "test" })
		makeIntent({
			intentDir,
			slug,
...
test("pre-intent verifier: clears when verified_at is stamped", async () => {
	if (!HAS_GIT) return
	await withTmpRepo("preintent-verified", async ({ repoRoot, intentDir, slug }) => {
		makeStudio({ repoRoot, studio: "test" })
		makeIntent({
			intentDir,
			slug,
...
test("tool-driven discovery: fires pre-units when template declares tool:", async () => {
	if (!HAS_GIT) return
	await withTmpRepo("tool-discovery-pre-units", async ({ repoRoot, intentDir, slug }) => {
		makeStudio({ repoRoot, studio: "test" })
		makeIntent({ intentDir, slug, studio: "test", mode: "continuous" })
		writeElaboration(intentDir, "design", { verified: true })
...
test("tool-driven discovery: clears when artifact lands at location:", async () => {
	if (!HAS_GIT) return
	await withTmpRepo("tool-discovery-cleared", async ({ repoRoot, intentDir, slug }) => {
		makeStudio({ repoRoot, studio: "test" })
		makeIntent({ intentDir, slug, studio: "test", mode: "continuous" })
		writeElaboration(intentDir, "design", { verified: true })
		const discoveryDir = join(
...
test("non-tool discovery: still gates on units > 0", async () => {
	if (!HAS_GIT) return
	await withTmpRepo("research-discovery", async ({ repoRoot, intentDir, slug }) => {
		makeStudio({ repoRoot, studio: "test" })
		makeIntent({ intentDir, slug, studio: "test", mode: "continuous" })
		writeElaboration(intentDir, "design", { verified: true })
