...
const { setHaikuRootForTests } = await import(
	"../src/state/shared.ts"
)
...
await test(
	"establish-all on a fresh intent writes baselines for all tracked files; tracking_classes counts match",
	async () => {
		...
		try {
			const result = await tool.handle({ intent_slug: slug, mode: "establish-all" })
			...
			assert.strictEqual(
				data.tracking_classes.knowledge,
				1,
				"1 knowledge file",
			)
			...
			const devBaseline = join(intentDir, "stages", "development", "baseline.json")
			...
			assert.ok(layoutKey in designData, "layout.html entry should be in baseline")
			assert.strictEqual(
			...
			assert.strictEqual(
				designData[layoutKey].tracking_class,
				"stage-output",
			)
		} finally {
...
await test(
	"establish-all on an intent with an existing baseline skips files whose SHA matches; baselines_skipped_existing reflects count",
	async () => {
		const { slug, root, intentDir } = makeIntentFixture({
			stages: [
				{
...
await test("establish-paths with a single path adds only that file's entry", async () => {
	...
	try {
		...
		assert.strictEqual(data.baselines_created, 1, "should create exactly 1 baseline")
...
await test(
	...
	async () => {
		...
		try {
			...
			assert.ok(!existsSync(baselinePath), "baseline should not exist before test")
...
await test(
	"establish-paths with intent.md path returns deny_list_match",
	async () => {
...
await test(
	"establish-paths with state.json path returns deny_list_match",
	async () => {
		const { slug, root } = makeIntentFixture({
			stages: [{ name: "development" }],
		})
...
await test(
	"establish-paths with drift-subsystem-internal path (baseline.json) returns deny_list_match",
	async () => {
		const { slug, root } = makeIntentFixture({
			stages: [{ name: "development" }],
		})
...
await test(
	"tracked_surface_empty warning when intent has no tracked files",
	async () => {
...
await test(
	"tool succeeds with a drift_disabled_warning when drift_detection: false",
	async () => {
		const { slug, root } = makeIntentFixture({
			driftDisabled: true,
			stages: [
		...
		try {
			...
			assert.strictEqual(data.ok, true, "should be ok even when kill-switch is on")
			// Must have created a baseline.
			assert.strictEqual(data.baselines_created, 1, "should still create a baseline")
