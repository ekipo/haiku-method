...
try {
	...
	test("prompt list returns empty array after migration to skills", () => {
		...
		assert.strictEqual(
			prompts.length,
			0,
			`Expected 0 prompts after migration, got ${prompts.length}`,
		)
	})
	...
	test("no core prompts registered", () => {
		...
		for (const name of corePrompts) {
			assert.ok(
				!names.includes(name),
				`Core prompt ${name} should NOT be registered (migrated to skill)`,
			)
		}
	})
	...
	test("no simple prompts registered", () => {
		...
		for (const name of simplePrompts) {
			assert.ok(
				!names.includes(name),
				`Simple prompt ${name} should NOT be registered (migrated to skill)`,
			)
		}
	})
	...
	test("no complex prompts registered", () => {
		...
		const names = prompts.map((p) => p.name)
		assert.ok(
			!names.includes("haiku:autopilot"),
			"Complex prompt haiku:autopilot should NOT be registered (migrated to skill)",
		)
	})
	...
	await testAsync("getPrompt throws for migrated prompt names", async () => {
		try {
			...
			assert.ok(
				e.message.includes("Unknown prompt"),
				`Expected 'Unknown prompt' error, got: ${e.message}`,
			)
		}
	})
