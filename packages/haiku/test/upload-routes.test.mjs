...
mkdirSync(join(intentDirPath, "stages", stageName, "artifacts"), { recursive: true })
...
writeFileSync(
  join(intentDirPath, "stages", stageName, "artifacts", "dashboard-layout.html"),
...
mkdirSync(join(archivedDirPath, "stages", stageName, "artifacts"), { recursive: true })
writeFileSync(
...
writeFileSync(
  ...
  JSON.stringify({ stage: stageName, status: "active", phase: "execute", visits: 0 }),
)

const lockedSlug = "test-locked-intent"
const lockedDirPath = join(haikuRoot, "intents", lockedSlug)
mkdirSync(join(lockedDirPath, "stages", stageName, "artifacts"), { recursive: true })
writeFileSync(
...
mkdirSync(join(sealedDirPath, "stages", stageName, "artifacts"), { recursive: true })
writeFileSync(
...
writeFileSync(
  ...
  JSON.stringify({ stage: stageName, status: "complete", phase: "gate", visits: 0 }),
)
...
const origSpawn = process.env.HOOK_INVOCATION_COUNT
...
async function run() {
  ...
  await test("Designer replaces a stage output: file written, action-log + audit-log stamped, no baseline update", async () => {
    ...
    const res = await fetch(`${baseUrl}/api/intents/${intentSlug}/uploads/stage-output`, {
      method: "POST",
    ...
    assert.strictEqual(data.baseline_updated, false, "baseline_updated must be false")
    assert.strictEqual(data.tick_will_observe, true, "tick_will_observe must be true")
    assert.ok(data.sha256, "sha256 should be present")
    assert.ok(data.bytes > 0, "bytes should be > 0")
    assert.ok(data.path.startsWith("stages/design/artifacts/"), "path should be in artifacts/")

    // File written to correct location.
    const dest = join(intentDirPath, "stages", stageName, "artifacts", "dashboard-layout.html")
    assert.ok(existsSync(dest), "Destination file should exist")
    const written = readFileSync(dest, "utf-8")
    assert.ok(written.includes("Dashboard v2"), "File should contain new content")
    ...
    assert.strictEqual(entry.author_class, "human-via-mcp", "author_class must be human-via-mcp")
    ...
    const auditLines = readFileSync(auditLog, "utf-8").split("\n").filter(Boolean)
    ...
    assert.strictEqual(auditEntry.user_instruction_excerpt, null, "SPA uploads have no chat instruction")

    // No baseline.json created.
    const baseline = join(intentDirPath, "stages", stageName, "baseline.json")
    assert.ok(!existsSync(baseline), "baseline.json must NOT be created by the upload endpoint")
  })
  ...
  await test("Replace preserves filename; uploaded file is renamed to the original target name", async () => {
    ...
    assert.ok(data.path.endsWith("dashboard-layout.html"), `path should end with dashboard-layout.html, got: ${data.path}`)

    // No extra "dashboard-v3.html" created.
    const extraFile = join(intentDirPath, "stages", stageName, "artifacts", "dashboard-v3.html")
    assert.ok(!existsSync(extraFile), "dashboard-v3.html must NOT be created in the worktree")
  })
  ...
  await test("mode=create with colliding filename returns 409 filename_collision", async () => {
    ...
    const { body, contentType } = buildMultipart(
      ...
      [{ name: "file", filename: "dashboard-layout.html", content: fileContent }],
    )
    ...
    const data = await res.json()
    assert.ok(data.error === "filename_collision" || data.code === "filename_collision", `Expected filename_collision, got: ${JSON.stringify(data)}`)
  ...
  await test("Upload exceeds size cap → 413 payload_too_large; no temp files left", async () => {
    ...
    const data = await res.json()
    assert.ok(data.error === "payload_too_large" || data.code === "payload_too_large", `Expected payload_too_large, got: ${JSON.stringify(data)}`)

    // No temp files left in artifacts/.
    const artifactsDir = join(intentDirPath, "stages", stageName, "artifacts")
    const tempFiles = readdirSync(artifactsDir).filter((f) => f.includes(".tmp"))
    assert.strictEqual(tempFiles.length, 0, `Expected no temp files, found: ${tempFiles.join(", ")}`)
  })
  ...
  await test("Locked worktree → 423 intent_locked", async () => {
    ...
    const res = await fetch(`${baseUrl}/api/intents/${lockedSlug}/uploads/stage-output`, {
      method: "POST",
      headers: { "Content-Type": contentType },
      body,
    })
    ...
    const data = await res.json()
    assert.ok(data.error === "intent_locked" || data.code === "intent_locked", `Expected intent_locked, got: ${JSON.stringify(data)}`)

    // No partial file should be left.
    const destFile = join(lockedDirPath, "stages", stageName, "artifacts", "new-file.html")
    assert.ok(!existsSync(destFile), "No file should be written for a locked intent")
  })
  ...
  await test("Archived intent → 404 intent_not_found", async () => {
    ...
    const res = await fetch(`${baseUrl}/api/intents/${archivedSlug}/uploads/stage-output`, {
      method: "POST",
      headers: { "Content-Type": contentType },
      body,
    })
    ...
    const data = await res.json()
    assert.ok(data.error === "intent_not_found" || data.code === "intent_not_found")
  })
  ...
  await test("Completed stage → 403 stage_not_writable", async () => {
    ...
    const res = await fetch(`${baseUrl}/api/intents/${sealedSlug}/uploads/stage-output`, {
      method: "POST",
      headers: { "Content-Type": contentType },
      body,
    })
    ...
    const data = await res.json()
    assert.ok(data.error === "stage_not_writable" || data.code === "stage_not_writable")
  })
  ...
  await test("Hook-bypass: PreToolUse hook script is not invoked during upload", async () => {
    ...
    assert.strictEqual(hookInvocations, 0, "No PreToolUse hook should have fired")
    ...
    const { body, contentType } = buildMultipart(
      ...
      [{ name: "file", filename: "mockup.png", content: newContent, contentType: "image/png" }],
    )
    ...
    assert.strictEqual(hookInvocations, 0, "Hook must not fire during SPA upload")
  })
  ...
  await test("Path traversal attack rejected with bad_target_path (400)", async () => {
    ...
    const data = await res.json()
    assert.ok(data.error === "bad_target_path" || data.code === "bad_target_path", `Expected bad_target_path, got: ${JSON.stringify(data)}`)
  })
  ...
  await test("PO uploads a knowledge file: written, action-log + audit-log stamped, no baseline update", async () => {
    ...
    const { body, contentType } = buildMultipart(
      ...
      [{ name: "file", filename: "competitive-analysis.md", content: fileContent }],
    )

    const res = await fetch(`${baseUrl}/api/intents/${intentSlug}/uploads/knowledge`, {
      method: "POST",
      headers: { "Content-Type": contentType },
      body,
    })
    ...
    assert.ok(existsSync(dest), "Knowledge file should exist at intent-scope knowledge/")
    const written = readFileSync(dest, "utf-8")
  ...
  await test("mode=create (implicit) with colliding knowledge filename returns 409", async () => {
    ...
    assert.strictEqual(res.status, 409)
    const data = await res.json()
    assert.ok(data.error === "filename_collision" || data.code === "filename_collision")
  })
