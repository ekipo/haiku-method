	...
	if (session.session_type === "review") {
		...
		if (session.intent) {
			...
			if (session.criteria?.length)
				steps.push({ label: "Criteria", key: "criteria" })
			if (session.knowledge_files?.length)
				steps.push({ label: "Knowledge", key: "knowledge" })
			if (session.output_artifacts?.length)
				steps.push({ label: "Artifacts", key: "artifacts" })
		}
...
export function ReviewSidebar({
	...
	const totalComments = Array.from(comments.values()).filter((c) =>
		c.trim(),
	).length
	...
	const handleSubmit = useCallback(
		async (decision: "approved" | "changes_requested") => {
			...
			const feedback = allComments
				.map((c) => `**${c.selectedText}:** ${c.comment}`)
				.join("\n\n")
			const annotations =
				allComments.length > 0 ? { comments: allComments } : undefined
	...
	return (
		<aside className="flex w-[280px] flex-shrink-0 flex-col border-r border-stone-800 bg-stone-950 p-6">
			<div className="mb-1 flex items-start justify-between">
				<div>
					<div className="text-lg font-bold tracking-wide text-stone-200">
						H·AI·K·U
					</div>
					<div
						...
						{steps.map((step, i) => (
							...
							>
								...
								<div>
									...
									{seenSteps.has(i) && i !== currentStep && (
										<div
											className={`text-xs ${comments.has(i) ? "text-yellow-400" : "text-stone-600"}`}
										>
...
function DecisionPanel({
	...
	return (
		<div>
			<div
				className={`mb-3 text-xs ${hasComments ? "text-yellow-400" : "text-teal-400"}`}
			>
				{hasComments
