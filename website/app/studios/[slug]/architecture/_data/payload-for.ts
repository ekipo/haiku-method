			...
			injection: [
				{
					hook: "MCP tool result",
					target: "agent's `tool_use_result`",
					what: "`action: manual_change_assessment` — pre-tick drift gate observed SHA divergence on tracked-surface files; agent must classify each finding before per-state dispatch resumes.",
				},
				{
					hook: "buildManualChangeAssessmentAction()",
					target: "agent prompt",
					what: "Each finding gets a stable `DRF-NN` id, a per-finding `legal_outcomes` list (filtered per AC-CO1: current-stage findings exclude `trigger-revisit`; per DATA-CONTRACTS.md §3.4: `file-removed` excludes `inline-fix`), and the agent receives instructions to call `haiku_classify_drift` with non-empty `agent_rationale` plus per-finding `rationale_excerpt` (AC-EE5).",
				},
			],
			action: "manual_change_assessment",
			summary:
				"out-of-band human edits detected on the tracked surface — agent classifies each finding into ignore / inline-fix / surface-as-feedback / trigger-revisit",
			payload: {
				action: "manual_change_assessment",
				stage: stageLower,
				tick_id: "tick-{slug}-{counter}-{iso}-{rand}",
				findings: [
					{
						finding_id: "DRF-01",
						path: ".haiku/intents/{slug}/stages/{stage}/artifacts/...",
				...
				mode: "continuous",
				legal_outcomes: {
					"<path>": ["ignore", "inline-fix", "surface-as-feedback"],
				},
				message:
					"Classify each finding via `haiku_classify_drift` (one Classification per finding, parallel-indexed). After the call returns, `haiku_run_next` resumes per-state dispatch.",
			},
			validations: [
				"Pre-tick drift gate produced ≥ 1 DriftFinding for the active stage",
				"Active stage's baseline.json was readable (non-corrupt) — corrupt baselines route to `error` instead",
				"Kill-switch `drift_detection: false` is NOT set in plugin settings",
			],
			writes: [
				{
					path: ".haiku/intents/{slug}/stages/{stage}/drift-assessments/DA-NN.json",
					change:
						"agent's classify call writes the `Assessment` record (append-only, durable per AC-G11)",
				},
				{
					path: ".haiku/intents/{slug}/stages/{stage}/baseline.json",
					change:
						"updated for terminal outcomes (`ignore`, `inline-fix`); held for non-terminal outcomes (pending-marker written instead — see AC-G4)",
				},
			],
			instructions:
				"**New (2026-04-30) — pre-tick drift gate dispatch.** Position in pre-tick chain: tamper-detection → feedback-triage → drift-detection → per-state dispatch. When the drift gate finds SHA divergence on the tracked surface (knowledge, stage artifacts, etc.), it short-circuits dispatch and emits this action. The agent receives all findings in one atomic batch (AC-G12) with per-finding legal-outcome filters; classification side effects (assessment record, baseline mutation, optional feedback / revisit) all happen inside `haiku_classify_drift` — the action is read-only metadata.",
		},
		"feedback-dispatch": {
			injection: [
				{
