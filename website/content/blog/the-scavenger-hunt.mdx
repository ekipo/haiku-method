...
<Callout tone="info" title="Prefer to click around than read?">
Every diagram below has a live, fully-interactive version. The [runtime architecture map](/prototype-stage-flow.html) renders the whole thing — actors, hooks, workflow phases, every `haiku_run_next` tick, every state write — and lets you click any node for its real source file. Each studio has its own view: [software](/studios/software/architecture/), [ideation](/studios/ideation/architecture/), [legal](/studios/legal/architecture/), [hwdev](/studios/hwdev/architecture/), and [20 others](/studios/). The rest of this post is the story; the map is the ground truth.
</Callout>
...
<ExpandableDiagram chart={`flowchart TB
    ...
    subgraph CC["Claude Code harness"]
        Agent["Agent (Claude)"]
        Hooks["Hooks<br/>prompt-guard · workflow-guard<br/>guard-workflow-fields · enforce-iteration<br/>inject-context · track-outputs<br/>quality-gate · subagent-context"]
    end
    ...
    subgraph Workflow["Workflow state on disk (.haiku/)"]
    ...
    Orchestrator -- "read + write<br/>(workflow side effects)" --> Workflow
...
<Callout tone="good" title="See it live">
This diagram is static; the [interactive runtime map](/prototype-stage-flow.html) isn't. Every actor, hook, MCP tool, hat, review-agent, and artifact chip on the live page is clickable and opens the real source file (`STAGE.md`, `hats/*.md`, `review-agents/*.md`, `discovery/*.md`, `outputs/*.md`). Swap studios from the picker in the top bar, or jump straight to any studio's architecture: `/studios/<slug>/architecture`.
</Callout>

## Stages and phases

Every studio is a named lifecycle template — a DAG of stages, each running its own phase workflow engine. H·AI·K·U ships with 24 studios (software, ideation, legal, hwdev, incident-response, marketing, hr, finance, and 16 more). The shape is the same across all of them; only the stage names and gate types differ. The default software studio has six stages in strict order: **inception → design → product → development → operations → security**. Every stage walks the same phase sequence: `pending → elaborate → execute → review → gate`. The orchestrator enforces the transitions; the agent can only move forward when the preconditions are actually met, not when it thinks they are.
...
The full state machine — six stages × five phases, every gate type, mode resolution, `/haiku:revisit` back-edges — lives in `packages/haiku/src/orchestrator.ts`. You don't have to take my word for it: the [runtime architecture map](/prototype-stage-flow.html) lays out every phase, every `haiku_run_next` action, every validation check, and every state write per stage. Toggle between continuous / discrete / hybrid / autopilot modes in the top bar to see how the gates change shape.
...
<Grid>
  <Card eyebrow="Guardrails" title="Block bad moves" accent="bad">
    ...
    - `guard-workflow-fields` — blocks direct edits to workflow-managed frontmatter
  ...
  <Card eyebrow="Context injection" title="Keep the agent honest" accent="good">
    - `inject-context` + `inject-state-file` — automatically injects workflow state so the agent never has to ask
...
<Grid>
  <Card eyebrow="Interactive" title="Runtime architecture map" accent="info">
    The full map — actors, hooks, workflow phases, MCP tools, knowledge pool, pre- and post-intent flows. Every chip clicks through to real source. [Open the map →](/prototype-stage-flow.html)
  </Card>
