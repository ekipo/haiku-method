...
When elaborating a greenfield or early-stage project without existing design knowledge, the workflow presents a design direction picker — a visual interface where the team reviews rendered previews of design archetypes (e.g., Brutalist, Editorial, Dense/Utilitarian, Playful/Warm) and either selects one as the final direction or asks the AI for more variants, optionally keeping a subset of the current archetypes and steering the next batch with comments. Pinned annotations on the chosen preview let the team capture pointed feedback at specific spots in the layout. The final selection produces a **design blueprint** — a structured artifact that defines the project's visual language in terms specific enough to guide wireframe generation and inform builder context.
