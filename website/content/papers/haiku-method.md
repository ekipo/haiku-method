...
**Scope routing.** A review agent in stage X can identify that a finding's root cause lives in stage Y — for example, a design reviewer noticing that an inception assumption was wrong. Rather than trying to fix the finding with stage X's hats (which cannot touch Y's artifacts coherently), the agent marks the finding with an `upstream_stage` field. The framework surfaces cross-stage findings to the human rather than routing them into an incorrect fix loop. The human decides: revisit upstream, reject the finding, or accept as-is. Same-stage findings flow into the fix loop automatically.
...
| **Upstream Finding** | A review finding whose root cause lives in a different stage than the one being reviewed. Reviewers pass `upstream_stage: "<stage>"` to `haiku_feedback` to route the finding. The workflow engine surfaces upstream findings to the human rather than dispatching the wrong stage's hats against the wrong artifacts. Human decides: revisit upstream, reject, or accept. |
