...
4. **Adversarial review** — Run the stage's review agents against the *produced artifacts* in two ordered tiers. **Tier 1 (spec gate):** any review agent marked `spec_gate: true` runs first, alone, to verify that the completed units collectively delivered exactly what the intent scoped — no scope creep, no missed criteria, no cross-unit drift. If spec findings are filed, the fix loop resolves them before quality review fires; this prevents burning token budget on quality concerns against an implementation that built the wrong thing. **Tier 2 (quality review):** once the spec gate is clear, quality agents (correctness, security, performance, architecture, test-quality, etc.) run in parallel. Agents from other stages included via `review-agents-include` also run in this tier. High-severity findings trigger targeted fixes before the stage can proceed.
